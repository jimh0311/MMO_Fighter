...
public:
	...
	enum enAYA_PACKET
	{
		eBUFFER_DEFAULT = 4096		// 패킷의 기본 버퍼 사이즈.
	};
	...
	CProtoBuffer&	operator << (UINT64 iValue);
	CProtoBuffer&	operator << (UINT iValue);
	...
	CProtoBuffer	&operator >> (UINT64 &i64Value);
	CProtoBuffer	&operator >> (UINT &i64Value);
